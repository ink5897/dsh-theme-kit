window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 文件由 lib/index.js 的 /dsh-ui-kit-wallpapers 路由从 wallpapers/ 目录提供；
    // 名字沿用用户起好的文件名（label 直接渲染，不再走 zh/en 字典）。
    const WALLPAPER_BASE = "/dsh-ui-kit-wallpapers";
    ...
    const WALLPAPER_BY_URL = new Map(
      WALLPAPER_GROUPS.flatMap((g) => g.items.map((it) => [it.url, it])),
    );

    // ── 纸纹纹理（选择纹理）：东方纸纹，平铺叠加在界面表面，给面板加纸质感 ──
    const TEXTURE_BASE = "/dsh-ui-kit-wallpapers/textures";
    const TEXTURES = [
    ...
    const tagId = "dsh-ui-kit/ThemesSection.module.css";
    if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
      const tag = document.createElement("style");
      tag.dataset.plugin = "dsh-ui-kit";
    ...
    function ThemePanel(props) {
      ...
      const isVideoBg = (v) => {
        const s = String(v || "");
        return s.startsWith("data:video/") || (/^\/dsh-ui-kit-wallpapers\/.*\.mp4$/i.test(s));
      };
    ...
    const inject = ["theme", "slots", "locale", "remote", "settingsScope", "timer"];
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.locale.register(NS, { zh, en }), "ui-kit: dictionaries");

      const theme = ctx.get("theme");
      const allThemes = PRESET_GROUPS.flatMap((g) => g.themes);
      const presetDisposers = allThemes.map((p) => theme.register(p));
      ctx.effect(() => () => presetDisposers.forEach((d) => d()), "ui-kit: preset themes");
      ...
      const STORAGE_KEY = "dsh-ui-kit:config";
      const readLocal = () => {
        try {
      ...
      const REMOTE_CONTRIBUTION = {
        package: "dsh-ui-kit",
        descriptors: [
          { id: "dsh-ui-kit#uiKit/getConfig", service: "uiKit", namespace: "uiKit", method: "getConfig",
            invocation: { kind: "direct" }, parameters: [], result: wireCodec("dsh-ui-kit#Config") },
          { id: "dsh-ui-kit#uiKit/setConfig", service: "uiKit", namespace: "uiKit", method: "setConfig",
            invocation: { kind: "direct" },
            parameters: [{ name: "config", wire: "config", source: "json", codec: wireCodec("dsh-ui-kit#Config") }],
            result: wireCodec("dsh-ui-kit#SetConfigResult") },
          { id: "dsh-ui-kit#uiKit/clearConfig", service: "uiKit", namespace: "uiKit", method: "clearConfig",
            invocation: { kind: "direct" }, parameters: [], result: wireCodec("dsh-ui-kit#SetConfigResult") },
        ],
      };

      const remoteReady = ctx.remote.$mount(REMOTE_CONTRIBUTION)
        .then(() => ctx.get("remote.uiKit"))
        .catch((error) => {
          console.warn("[dsh-ui-kit] remote mount failed:", error);
          return null;
        });
      const callRemote = async (method, ...args) => {
        try {
          ...
          console.warn("[dsh-ui-kit] host remote call failed:", error);
          return null;
        }
      };
      ...
      const normalizeBackground = (raw) => {
        ...
        return {
          ...
          glass: typeof b.glass === "number"
            ...
            : b.glass === "clear" ? 0 : b.glass === "liquid" ? 50 : b.glass === "frost" ? 100 : 1,
      ...
      const applyBackground = () => {
        ...
        const isVideo = typeof background.image === "string" &&
          (background.image.startsWith("data:video/") ||
            /^\/dsh-ui-kit-wallpapers\/.*\.mp4$/i.test(background.image));
      ...
      ctx.effect(() => offTheme, "ui-kit: background theme sync");
